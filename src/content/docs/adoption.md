...
You do not have to describe the whole product, or install anything to try it: every kind of file has a [copy-paste template](/templates/). [ProductShape](https://github.com/juangcarmona/productshape) is the reference implementation, and it is optional.
...
Those four frontmatter fields and four sections are all a business rule needs; links to other files can come later. The other ten kinds have the same ready shape on the [templates page](/templates/).
...
Full PDaC is a set of rules to meet, not a level to reach: the [specification](/spec/) says what they are.
