...
description: Product requirements in versioned Markdown that delivery documents cite by stable ID and digest. When cited text changes, tools detect documentation drift in the recorded citations.
template: splash
hero:
...
{/* Canonical PDaC definition paragraph — verbatim from the spec repository's
    README (canonical-pdac-definition markers). Do not paraphrase or edit here. */}
...
The specification is a v0.1 draft under public RFC, weeks old and labeled accordingly, extracted from one working reference implementation. Independent conformance, a second implementation and external pilots are the next proof points, and they are release gates for v1, not assumed achievements. The [known limits](/known-limits/) page names every gap we know about.
